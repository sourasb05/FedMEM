# import torch
from src.PerMFL.UserPerMFL import UserPerMFL
...
class PerMFL():
    def __init__(self, device, model, args, i):
        
        
        self.tot_group_samples = [ [] for _ in range(args.num_teams) ]
        self.tau = [ [] for _ in range(args.num_teams) ]
        self.model_name = args.model_name
        self.w_bar = copy.deepcopy(model)
        self.theta_bar = copy.deepcopy(model)
        self.local_model = copy.deepcopy(model)  # initialize global model
        
        self.team = []
        for i in range(0,args.num_teams):
            self.team.append(copy.deepcopy(model))
        
        self.users = [[] for _ in range(args.num_teams)]
        self.participated_devices = []
        
        self.device_train_acc = [[] for _ in range(args.num_teams)]
        self.device_test_acc = [[] for _ in range(args.num_teams)]
        self.device_train_loss = [[] for _ in range(args.num_teams)]
        self.device_test_loss = [[] for _ in range(args.num_teams)]

        self.team_train_acc = [[] for _ in range(args.num_teams)]
        self.team_test_acc = [[] for _ in range(args.num_teams)]
        self.team_train_loss = [[] for _ in range(args.num_teams)]
        self.team_test_loss = [[] for _ in range(args.num_teams)]
        ...
        self.team_iters = args.team_epochs
        ...
        self.group = args.num_teams
        
        self.exp_no = args.exp_no
        self.analysis = args.analysis

        self.p_teams = args.p_teams
        self.team_list = list(range(args.num_teams))
        ...
        """  
          data[0] has clients
          data[1] training data of users
          data[2] test data of users
          data[3] client divided in teams
        ...
        self.tot_users=args.tot_users
        self.groups = data[3]
        self.model_initializer()
        
        for grp in range(len(data[3])):    ## iterate over number of teams 
            total_users = len(data[3][grp])   ## number of user in a team
            self.group_sample = 0
            for i in range(total_users):        
                id, train, test = read_user_data(data[3][grp][i], data)
                user = UserPerMFL(device, train, test, model, args, i)
        ...
        for grp in range(args.num_teams):
    ...
    def test_personalized_model(self):
        ...
        for c in self.participated_devices:
            ...
            num_samples.append(ns)
            i += 1
        print("Num_participated_device_test :",i)
        # ids = [c.id for c in self.users[grp]]
    ...
    def train(self):
        ...
        for iters in trange(self.num_glob_iters, desc="Global rounds"):  # t = 1.... T-1
            list_devices = []
            ...
            for grp in tqdm(self.sub_group, desc="Processing team"):  # for group 0 and 1
                # print("---Computing parameters for group :[", grp, "]")
                self.set_team_parameter(grp)

                
                for team_iter in trange(self.team_iters, desc="Teams rounds"):  # k = 1 .. K-1
