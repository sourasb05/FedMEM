...
class DemLearn(Dem_Server):
    def __init__(self, experiment, device, dataset, algorithm, model, batch_size, learning_rate, beta, L_k,
        ...
        # print(self.sub_data)
        # input("press at line 36 in ServerDemLearn")
        if (self.sub_data):
            print(">>> Cutoff >>>>")
            randomList = self.get_partion(self.total_users)
            # print(randomList)
            # input("press at 39 in serverDemLearn")
    ...
    def train(self):
        ...
        for i in range(self.num_glob_iters):
            ...
            stats = self.evaluating_clients(i, mode="spe")
            # print("stats spe:",stats)
            #input("press")
            self.cs_avg_data_test.append(stats[0])
            self.cs_avg_data_train.append(stats[1])
            self.cs_loss_test.append(stats[2])
            tqdm.write('============= Test Client Models - Generalization ============= ')
            stats = self.evaluating_clients(i, mode="gen")
            self.cg_avg_data_test.append(stats[0])
            self.cg_avg_data_train.append(stats[1])
            self.cg_loss_test.append(stats[3])
    ...
    def save_results_dem(self):
        ...
        rs_path = current_directory + "/results/100_fmnist_mclr/" + str(self.times) + "_" + "fmnist" + "_" + "" + ".h5" 
        print(rs_path)
        write_file(file_name=rs_path, root_test=root_test, root_train=root_train,
                   cs_loss_test=self.cs_loss_test, cg_loss_test=self.cg_loss_test,
