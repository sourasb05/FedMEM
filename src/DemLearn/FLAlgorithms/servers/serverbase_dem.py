...
class Dem_Server(Server):
    ...
    def g_test(self, gr, mode="spe"):  # mode spe: specialization, gen: generalization
        ...
        return ids, groups, num_samples, tot_correct, ls
    ...
    def gc_test(self,c_model):
        ...
        losses = []
        for c in self.users:
            stats = c.test_gen(c_model)
            tot_correct.append(stats[0] * 1.0)
            num_samples.append(stats[1])
            losses.append(stats[2]*1.0)

        return np.sum(tot_correct),np.sum(num_samples),  np.sum(losses)/len(self.users)
    ...
    def c_test(self, i, mode="spe"):  # mode spe: specialization, gen: generalization
        ...
        for c in self.users:
            if (mode == "spe"):
                ...
                tot_correct.append(stats[0] * 1.0)
                num_samples.append(stats[1])
                clients_acc.append(stats[0] / stats[1])
                losses_spe.append(stats[2]*1.0)
                
                #input("loss press line 497 serverbase_dem")
            elif (mode == "gen"):
                # c_test_model = copy.deepcopy(c.model)
                # ct, ns = self.gc_test(c_test_model)  # Test client as testing group approach in gen mode
                # print("mode: ",mode)
                # input("going gc_test")
                stats = self.gc_test(c.model)  # Test client as testing group approach in gen mode

                tot_correct.append(stats[0] * 1.0)
                num_samples.append(stats[1])
                clients_acc.append(stats[0] / stats[1])
                losses_gen.append(stats[2]*1.0)

        # print("Testing Acc Client:", clients_acc )

        if (mode == "spe"):
            self.cs_data_test[i, :] = clients_acc
            # self.cs_loss_test[i, :] = losses

            # print(self.cs_loss_test)
        elif (mode == "gen"):
            self.cg_data_test[i, :] = clients_acc
            # self.cg_loss_test[i, :] = losses
        # self.client_data_test.append(clients_acc)

        ids = [c.id for c in self.users]
        groups = [c.group for c in self.users]

        # print(np.mean(clients_acc))
        #print(np.mean(losses_spe))
        #print(np.mean(losses_gen))

        return ids, groups, num_samples, tot_correct, losses_spe, losses_gen
    ...
    def evaluating_clients(self, i, mode="spe"):  # mode spe: specialization, gen: generalization
        ...
        spe_test_loss = np.mean(stats[4])
        gen_test_loss = np.mean(stats[5])
        tqdm.write('Mode {} At round {} AvgC. testing accuracy: {} AvgC. spe test loss: {} AvgC. gen test loss {} '.format(mode, i, test_acr, spe_test_loss, gen_test_loss))
        ...
        # elif (mode == "gen"):
        #     train_acr = []
        return test_acr, train_acr, spe_test_loss, gen_test_loss
